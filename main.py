...
def registered_keyboard_bn():
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ Ami Register Korlam - ID Pathan", callback_data='registered')]])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'lang_en':
        ...
        n = chr(10)
        welcome_text = "⚽ ELITE ANALYSIS HUB E APNAKE SAGOTO" + n + n + "📊 Protidin football statistics ebong data-driven predictions." + n + "🏆 Premier League, La Liga, Champions League coverage." + n + n + "🎯 VIP MODEL OUTPUTS KIVABE PABEN:" + n + "1️⃣ Nichey dewa link e click kore partner platform e register korun." + n + "2️⃣ Prothom transaction complete korun (Minimum 500 BDT)." + n + "3️⃣ Ekhaney fire eshe 'Ami Register Korlam' e click korun." + n + "4️⃣ Apnar Partner Site ID number ta ekhane likhun." + n + "5️⃣ Amader team 24 ghontar moddhe verify korbe ebong apnar private access link pathabe." + n + n + "🔗 LINK: " + REFERRAL_LINK + n + n + "⚠️ Important: Shudhu register korlei hobe na. Prothom transaction complete korar por e approval dewa hobe."
        if WELCOME_IMAGE_URL:
        ...
        if lang == 'bn':
            caption = "📈 Ekhane kichu high-odds VIP analysis result dekhun! Access pete apnar Partner Site ID number ekhane likhun."
        else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.isdigit() or len(text) < 3 or len(text) > 20:
        if lang == 'bn':
            await update.message.reply_text("❌ Shudhu numeric Partner Site ID din (3 theke 20 digit er moddhe).")
        else:
    ...
    if lang == 'bn':
        await update.message.reply_text("✅ Apnar information 24 ghontar moddhe check kora hobe. Approve hoy gele apnar private VIP link pathano hobe.")
    else:
...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        n = chr(10)
        if lang == 'bn':
            msg = "🎉 Apnar ID approve hoyeche!" + n + n + "⚠️ SECURITY NIYOM:" + n + "• Ei link shudhu APNAR jonne." + n + "• Eti shudhu EKBAR use kora jabe." + n + "• 1 ghontar moddhe use korun nahole expire hoye jabe." + n + "• Onno keo share korle, TARA dukbe, APNI na." + n + n + "🔗 APNAR VIP LINK:" + n + invite_link.invite_link + n + n + "👉 Taratari click korun ebong channel e join korun!"
        else:
